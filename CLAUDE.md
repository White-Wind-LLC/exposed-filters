...
3. **Field resolution** (`jdbc/QueryFilterExtensions.kt`). `Query.applyFiltersOn(Table, filter)` uses reflection (`Table::propertyToColumnMap`) to map **Kotlin property names** (e.g. `warehouseId`) to columns — not SQL column names. For a non-`Table` `ColumnSet` (Join/Alias), it falls back to SQL column names, plus the alias label of every computed field (`IExpressionAlias`) a `QueryAlias` projects — Exposed keeps those out of `columns`, so `sourceFields()` walks the sources itself (a `Join`'s parts through reflection on its internal `joinParts`, like `referee`) and reads each one as `alias.label` via `QueryAlias.get`. Never satisfy a filter by wrapping the caller's query in an outer `SELECT * FROM (…)`: that changes the columns result rows are keyed by. The same name from two sources — column or computed — is rejected as ambiguous. `applyFilters(Map<String, ExpressionWithColumnType<*>>, ...)` is the low-level escape hatch for unions, aliases, and computed expressions.
