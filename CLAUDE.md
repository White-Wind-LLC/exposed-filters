...
3. **Field resolution** (`jdbc/QueryFilterExtensions.kt`). `Query.applyFiltersOn(Table, filter)` uses reflection (`Table::propertyToColumnMap`) to map **Kotlin property names** (e.g. `warehouseId`) to columns — not SQL column names. A column the table registers without a property (a CTE modelled as a `Table`, `registerColumn` at runtime) is added under its SQL name by the internal `Table.fieldMap()`; a property name wins a collision, and a column a property holds is never exposed under its SQL name too. The public `propertyToColumnMap()` stays property-only. The `memberProperties` scan behind it is cached **per class** (`TableColumnProperties.kt`, a `ClassValue` of accessible `KProperty1`s whose declared type admits a `Column`); each call still reads the properties off the given instance. Never cache per table instance: every `Column` references its table, so a weak-keyed instance→map cache never releases runtime tables or `Alias`es. Nested paths look the referenced table up through `fieldMap()` as well. For a non-`Table` `ColumnSet` (Join/Alias), it falls back to SQL column names, plus the alias label of every computed field (`IExpressionAlias`) a `QueryAlias` projects — Exposed keeps those out of `columns`, so `sourceFields()` walks the sources itself (a `Join`'s parts through reflection on its internal `joinParts`, like `referee`) and reads each one as `alias.label` via `QueryAlias.get`. Never satisfy a filter by wrapping the caller's query in an outer `SELECT * FROM (…)`: that changes the columns result rows are keyed by. The same name from two sources — column or computed — is rejected as ambiguous. `applyFilters(Map<String, ExpressionWithColumnType<*>>, ...)` is the low-level escape hatch for unions, aliases, and computed expressions.
4. **Dot-path fields** (`predicateForField`). Two unrelated features share `foo.bar` syntax:
   ...
   - Otherwise the base must resolve to a referenced table, and an `EXISTS` subquery is built against it. `resolveReference` (reflective access to Exposed's `referee` field/getter) is tried first; when it comes back empty, `FilterOptions.referenceResolver` — a caller-supplied `(Column<*>) -> ReferenceInfo?`, `null` by default — gets its turn. That fallback exists for columns whose foreign key is logical only: a modular codebase stores a plain `uuid` when the target table lives in a module the owner must not depend on. Keep the order — a declared reference must always win, so the resolver never changes behavior for callers that don't set it. **Only one level of reference nesting is supported** (`ref.field`, not `a.b.c`).
   - A column of a table `Alias` or a `QueryAlias` is a clone without `referee` (`QueryAlias` clones onto `table.alias(queryAlias.alias)`, so both have an `Alias` table). `originalColumn()` unwraps it through `Alias.originalColumn`, nested aliases included; both `resolveReference` and `referenceResolver` see the original, while the subquery keeps comparing against the clone — the original's table isn't in the outer `FROM`.
   - Once the target table is known, `FilterOptions.nestedFieldResolver` — `(Table, String) -> NestedFieldProjection?`, `null` by default — may replace what the path reads: the projection supplies the subquery's source and the expression the predicate is built against. A source that joins the target table keeps exposing its id column; one that renames or replaces it (alias, subquery, CTE, temporary table) must set `idExpression`, and a source that does neither is rejected with `require` before any SQL is built — never let the target table's id fall through to a source that lacks it, since it would silently bind to the outer query. This one is consulted for **every** nested path, including a declared reference, because the reference is not what is being replaced — the column behind it is. Do not move it behind the `referee` check to match `referenceResolver`: that would make it dead for exactly the physical-FK columns it exists to serve.
5. **`WHERE` vs `HAVING`** (`jdbc/QueryFilterAggregates.kt` `splitByClause`). Every entry point splits the tree before building predicates: a predicate whose base field is an aggregate goes to `andHaving`, the rest to `andWhere`. AND conjuncts split one by one (nested ANDs flatten); an OR/NOT group touching an aggregate moves to `HAVING` whole — never split it, that changes its meaning. A tree without aggregates is passed through untouched so its SQL doesn't change. Aggregates are detected by a reflective walk of the expression tree (`expressionKind`) for Exposed's aggregate classes; it must stop at `Column` and at `IExpressionAlias.aliasOnlyExpression()` wrappers — `QueryAlias.get` returns those for a subquery's computed field and they hold the subquery's `SUM` in a field, but read an already computed value. Unrecognized aggregates (`CustomFunction`) are declared via `FilterOptions.aggregateFields`. A `WindowFunctionDefinition` fails loudly. Never solve this by wrapping the query in an outer `SELECT` (see point 3).
6. **Value coercion** (string → column type). Built-in handling covers Int/Long/Short/Double/String/UUID/Boolean/enums/date/time (both `java.time` and `kotlinx.datetime`). Users can inject `ColumnMappersModule` to override or add types; custom mappers are tried **before** built-ins, in **reverse registration order** (last added first).
   ...
   - `CustomEnumerationColumnType` exposes no enum class, and its `fromDb`/`toDb` lambdas are `invokedynamic` with fully erased signatures - reflecting on the lambda cannot recover the class, don't try. Instead the class comes from the Kotlin property declaring the column (`val status: Column<Status>` → `Status`), via the same per-class property cache `propertyToColumnMap()` uses; `Column<List<Status>>` is unwrapped one level deeper for array columns, and `Alias` columns resolve through `Alias.delegate`. The result is memoized per column-type instance (identity-keyed - `CustomEnumerationColumnType` doesn't override `equals`/`hashCode`) because the scan is far too slow to repeat per filter value. When the class cannot be found (a computed expression via `applyFilters(map)`, not a real column), the filter fails loudly rather than falling back to `fromDb` - resolving through `fromDb` would mean the client had to know the database representation, which defeats the library's purpose.
