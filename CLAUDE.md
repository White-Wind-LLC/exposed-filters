...
4. **Dot-path fields** (`predicateForField`). Two unrelated features share `foo.bar` syntax:
   ...
   - Once the target table is known, `FilterOptions.nestedFieldResolver` — `(Table, String) -> NestedFieldProjection?`, `null` by default — may replace what the path reads: the projection supplies the subquery's source (the target table joined to another one, and it must still expose the referenced id column) and the expression the predicate is built against. This one is consulted for **every** nested path, including a declared reference, because the reference is not what is being replaced — the column behind it is. Do not move it behind the `referee` check to match `referenceResolver`: that would make it dead for exactly the physical-FK columns it exists to serve.
